...
const HeaderTitel = () => {
  ...
  useEffect(() => {
    if (ref.current) {
      const job = setInterval(() => {
        setTransition((prev) => prev - (ref.current?.offsetHeight ?? 0));
  ...
  useEffect(() => {
    if (
      Math.abs(transition) >=
      ref.current!.offsetHeight * headertitels.length
    ) {
      setDuration("");
      setTransition(0);
    }
    ...
    <div>
      {headertitels.map((titels, index) => (
        <p
          ...
          className={cn(
            "text-black text-5xl font-bold lg:translate-x-8 transform",
            duration
          )}
          key={index}
        >
